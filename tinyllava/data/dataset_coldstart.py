...
from ..utils.data_utils import get_jpg_files_os,get_png_files_os
...
class HBSupervisedDataset(LSDataset):
    """Dataset for supervised fine-tuning."""

    def __init__(self, data_path: str,
                 tokenizer: transformers.PreTrainedTokenizer,
                 data_args: DataArguments):
        super(HBSupervisedDataset, self).__init__()
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources:
            ...
            video_data = video_data['video'].permute(1, 0, 2, 3)#(num_frames,3,w,h)
        ...
        elif 'scene_id' in sources:
            scene_file = self.list_data_dict[i]['scene_id']
            scene_folder = os.path.join(self.data_args.video_folder, scene_file)
            color_folder=scene_folder+'/color'
            color_images=get_jpg_files_os(color_folder)
            depth_folder=scene_folder+'/depth'
            depth_images=get_png_files_os(depth_folder)

            assert len(color_images)==len(depth_images) , "length of color images and depth images must be equal" 
            total_frames = len(color_images)
            assert self.num_frames >0 , "num frames > 0"
            color_images_files = color_images[:self.num_frames] #torch.Size([8, 3, W, H])
            depth_images_files = depth_images[self.num_frames] 
            color_images=[]
            
            transform = transforms.Compose([
                transforms.ToTensor() 
            ])   
            for color_image_file in color_images_files:
                image = Image.open(color_image_file).convert('RGB')
                image=transform(image)
                color_images.append(image)
            color_images=torch.stack(color_images)
            videos = []
            for video in color_images:
                video = self.video_preprocess(video)
                videos.append(video)
            videos = torch.stack(videos) #(num frames,3,384,384)
            data_dict['video']=videos
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    train_dataset = HBSupervisedDataset(tokenizer=tokenizer,
